...
class InterferometryProcessor:
    def __init__(self, subswath, output_folder, wkt, s1a_bursts, s1b_bursts):
    ...
    def run(self, master_path, slave_path, output_base_name):
        ...
        for pol in ["VV", "VH"]:
            ...
            bursts = self.s1a_bursts if "S1A" in master.getName() else self.s1b_bursts
            split_params.put('firstBurstIndex', self.java_int(int(bursts[0])))
            split_params.put('lastBurstIndex', self.java_int(int(bursts[1])))
            ...
            bursts_s = self.s1a_bursts if "S1A" in slave.getName() else self.s1b_bursts
            split_params_s.put('firstBurstIndex', self.java_int(int(bursts_s[0])))
            split_params_s.put('lastBurstIndex', self.java_int(int(bursts_s[1])))
