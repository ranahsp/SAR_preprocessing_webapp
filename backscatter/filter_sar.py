...
class Filter_sar:
    @staticmethod
    def aoi_orbit(safe_path, aoi_wkt, subswath):
        try:
            ...
            print(f" {os.path.basename(safe_path)} -> Orbit: {rel_orbit} | Subset size: {width}x{height}", flush=True)
            ...
            print(f" Error processing {os.path.basename(safe_path)}: {e}", flush=True)
            return None

    @staticmethod
    def filter_images(safe_paths, aoi_wkt, subswath):
        filtered = {}
        target_orbit = None

        for filename, path in safe_paths.items():
            
            rel_orbit = Filter_sar.aoi_orbit(path, aoi_wkt, subswath) 

            if rel_orbit:
                if not target_orbit:
                    target_orbit = rel_orbit
                    print(f"Reference Orbit Selected: {target_orbit}", flush=True)
                if rel_orbit == target_orbit:
                    filtered[filename] = path
                    print(f" Accepted: {filename}", flush=True)
                else:
                    print(f" Skipped: Orbit mismatch ({rel_orbit})", flush=True)

        print(f"\n Final selected images: {len(filtered)} in Orbit {target_orbit}", flush=True)
        return filtered
